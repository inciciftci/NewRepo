...
export function initDatabase() {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_notes_country_id ON notes(country_id);
  ...
  if (authCount.count === 0) {
    ...
    db.prepare('INSERT INTO auth (password_hash) VALUES (?)').run(hash);
...
function seedCountries() {
  ...
  const insert = db.prepare('INSERT INTO countries (name, code) VALUES (?, ?)');
  const insertMany = db.transaction((countries) => {
    for (const country of countries) {
      insert.run(country.name, country.code);
    }
  });
